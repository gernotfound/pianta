  ...
  return (
    <div className="section active fade-in" style={{ paddingBottom: '100px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}>
          <button className="btn btn-outline" style={{ padding: '8px 12px', margin: 0 }} onClick={() => navigate(-1)}>⬅️ Indietro</button>
          <div>
              <button className="btn btn-outline" style={{ padding: '8px 12px', margin: '0 5px 0 0' }} onClick={() => setIsQrModalOpen(true)}>🏷️ QR</button>
              <button className="btn btn-outline" style={{ padding: '8px 12px', margin: '0 5px 0 0' }} onClick={() => navigate(`/edit-plant/${plant.id}`)}>✏️ Modifica</button>
              <button className="btn btn-danger" style={{ padding: '8px 12px', margin: 0 }} onClick={handleDelete}>🗑️</button>
          </div>
      </div>
      ...
      <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
          <div style={{ display: 'flex' }}>
              <div style={{ flex: 1 }}>
                  {plant.photo ? (
                      <img src={imgMain} alt="Pianta" style={{ width: '100%', height: '200px', objectFit: 'cover' }} />
                  ) : (
                      <div style={{ width: '100%', height: '200px', background: 'var(--surface-border)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-muted)' }}>Nessuna Foto</div>
                  )}
              </div>
              {plant.fruitPhoto && (
                  <div style={{ flex: 1, borderLeft: '2px solid var(--bg)' }}>
                      <img src={imgFruit} alt="Frutto" style={{ width: '100%', height: '200px', objectFit: 'cover' }} />
                  </div>
              )}
          </div>
          
          <div style={{ padding: '20px' }}>
              <h1 style={{ margin: '0 0 5px 0', color: 'var(--primary)', fontSize: '24px' }}>{plant.name}</h1>
              {plant.scientific && <h3 style={{ margin: '0 0 15px 0', color: 'var(--text-muted)', fontStyle: 'italic', fontWeight: 'normal' }}>{plant.scientific}</h3>}
              
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: '10px' }}>
                  <div className="info-widget">
                      <div className="widget-label">🌱 Origine</div>
                      <div className="widget-value">{plant.origin || 'N/D'}</div>
                  </div>
                  <div className="info-widget">
                      <div className="widget-label">🪴 Sistemazione</div>
                      <div className="widget-value">{plant.placement || 'N/D'} {plant.potSize ? `(${plant.potSize}L)` : ''}</div>
                  </div>
                  <div className="info-widget">
                      <div className="widget-label">🪨 Substrato</div>
                      <div className="widget-value">{plant.soil || 'N/D'}</div>
                  </div>
                  <div className="info-widget">
                      <div className="widget-label">🧪 pH Ottimale</div>
                      <div className="widget-value">{plant.phMin || '?'} - {plant.phMax || '?'}</div>
                  </div>
                  <div className="info-widget">
                      <div className="widget-label">🌡️ Temperature</div>
                      <div className="widget-value">{plant.minTemp || '?'}°C / {plant.maxTemp || '?'}°C</div>
                  </div>
              </div>
          </div>
      </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '30px', marginBottom: '15px' }}>
          <h2 style={{ margin: 0, color: 'var(--primary)' }}>Diario Eventi</h2>
          <button className="btn btn-blue" style={{ margin: 0, padding: '8px 15px' }} onClick={() => setIsEventModalOpen(true)}>➕ Aggiungi</button>
      </div>

      {sortedLogs.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '30px', background: 'var(--surface)', borderRadius: '12px' }}>
              <p style={{ color: 'var(--text-muted)' }}>Nessun evento registrato. Aggiungi il primo evento!</p>
          </div>
      ) : (
