    ...
    if (typeof plantsDatabase !== 'undefined' && plantsDatabase.length > 0) {
        if (typeof Swal !== 'undefined') {
            Swal.fire({
                ...
                text: "⚠️ ATTENZIONE: Questa operazione sovrascriverà il giardino attualmente aperto. Assicurati di aver esportato il giardino attuale prima di procedere!",
                icon: 'warning',
...
async function loadZipProfile(file) {
    ...
    try {
        ...
        if (typeof Swal !== 'undefined') {
            Swal.fire({icon: 'error', title: 'Errore Backup', text: "Errore nel caricamento del file ZIP: " + errorMessage, confirmButtonColor: '#d32f2f'});
        } else {
...
async function exportData() {
    ...
    try {
        ...
        if (typeof Swal !== 'undefined') {
            Swal.fire({icon: 'error', title: 'Errore Esportazione', text: "Si è verificato un errore, forse la memoria del dispositivo è piena. Riavvia l'app e riprova.", confirmButtonColor: '#d32f2f'});
        } else {
...
function exportToCSV() {
    if (!plantsDatabase || plantsDatabase.length === 0) {
        if (typeof Swal !== 'undefined') Swal.fire({icon: 'info', title: 'Nessun Dato', text: 'Non c\'è nessuna pianta da esportare nel file Excel!', confirmButtonColor: '#1976d2'});
        return;
    }
